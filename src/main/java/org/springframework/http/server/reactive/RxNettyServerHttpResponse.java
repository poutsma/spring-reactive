...
public class RxNettyServerHttpResponse extends AbstractServerHttpResponse {
	...
	protected Mono<Void> writeWithInternal(Publisher<DataBuffer> body) {
		...
		return RxJava1ObservableConverter.from(this.response.write(content, bb -> bb instanceof Flushable)).then();
	}
	...
	private ByteBuf toByteBuf(DataBuffer buffer) {
		...
		return (buffer instanceof Flushable ? new FlushableByteBuf(byteBuf) : byteBuf);
	}
	...
	private class FlushableByteBuf extends CompositeByteBuf implements Flushable {

		public FlushableByteBuf(ByteBuf byteBuf) {
